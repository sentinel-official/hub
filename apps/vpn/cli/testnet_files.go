...
func TestnetFilesCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a VPNd testnet",
...
Example:
	vpnd testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "vpnd",
		"Home directory of the node's daemon configuration",
	)
	cmd.Flags().String(flagNodeCliHome, "vpncli",
...
func initTestnet(config *tmConfig.Config, cdc *codec.Codec) error {
	...
	vpnConfig := serverConfig.DefaultConfig()
	vpnConfig.MinGasPrices = viper.GetString(server.FlagMinGasPrices)

	var (
		accs     []vpn.GenesisAccount
		genFiles []string
	)
	...
	for i := 0; i < numValidators; i++ {
		...
		prompt := fmt.Sprintf(
			"Password for account '%s' (default %s):", nodeDirName, vpn.DefaultKeyPass,
		)
		...
		accs = append(accs, vpn.GenesisAccount{
			Address: addr,
		...
		signedTx, err := txBldr.SignStdTx(nodeDirName, vpn.DefaultKeyPass, tx, false)
		if err != nil {
			_ = os.RemoveAll(outDir)
			return err
		}
		...
		vpnConfigFilePath := filepath.Join(nodeDir, "config/vpnd.toml")
		serverConfig.WriteConfigFile(vpnConfigFilePath, vpnConfig)
	}
...
func initGenFiles(
	cdc *codec.Codec, chainID string, accs []vpn.GenesisAccount,
	genFiles []string, numValidators int) error {

	appGenState := vpn.NewDefaultGenesisState()
