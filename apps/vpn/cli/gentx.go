...
func GenTxCmd(ctx *server.Context, cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: fmt.Sprintf(`This command is an alias of the 'vpnd tx create-validator' command'.
...
`, defaultAmount, defaultCommissionRate, defaultCommissionMaxRate, defaultCommissionMaxChangeRate),
	...
	cmd.Flags().String(tmCli.HomeFlag, vpn.DefaultNodeHome, "node's home directory")
...
func accountInGenesis(genesisState vpn.GenesisState, key csdkTypes.AccAddress, coins csdkTypes.Coins) error {
