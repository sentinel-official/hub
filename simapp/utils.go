...
func GenVpnGenesisState(cdc *codec.Codec, r *rand.Rand, accs []simulation.Account, ap simulation.AppParams, genesisState map[string]json.RawMessage) {
	...
	vpnGenesis := vpn.GenesisState{
		Params: vpn.NewParams(
			func(r *rand.Rand) uint64 {
				var v uint64
				ap.GetOrGenerate(cdc, vpnsim.FreeNodesCount, &v, r,
					func(r *rand.Rand) {
						v = uint64(simulation.RandIntBetween(r, 10, 1000))
					})
				return v
			}(r),
			func(r *rand.Rand) sdk.Coin {
				var v sdk.Coin
				ap.GetOrGenerate(cdc, vpnsim.Deposit, &v, r,
					func(r *rand.Rand) {
						v = sdk.NewInt64Coin(sdk.DefaultBondDenom, int64(simulation.RandIntBetween(r, 1, 1e3)))
					})
				return v
			}(r),
