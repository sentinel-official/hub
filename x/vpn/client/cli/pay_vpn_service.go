...
func PayVpnServiceCommand(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			vpnAddress := viper.GetString(flagVPNAddress)
			...
			vpnAddressBytes, err := csdkTypes.AccAddressFromBech32(vpnAddress)
			...
			msg := vpn.NewMsgPayVpnService(coins,vpnAddressBytes,from,account.GetPubKey())
	...
	cmd.Flags().String(flagVPNAddress, "", "VPN node address")
